...
import { QueryKeysFinalReport } from '../api/useFinalReport';
...
const useUploadFile = () => {
  ...
  const updateArticle = async (file: any, id: string, data: any) => {
    const body = {
      file: file,
      id: id,
      termId: termId,
    ...
    const submitter = (await axiosFile.put(`${env.API_URL}/api/v1/articles/${id}`, body, {
      onUploadProgress: (axiosLoading: AxiosProgressEvent) => {
        setValueLoading(axiosLoading.progress);
      },
    })) as ResponseType;
  ...
  const submitFinalReport = async (file: any) => {
    ...
    if (submitter.success === true) {
      setCurrentFile(null);
      enqueueSnackbar(submitter.message, { variant: 'success' });
      queryClient.resetQueries({ queryKey: [QueryKeysFinalReport.FinalReport] });
    }
    if (submitter.status < 500 && submitter.status >= 400) {
      enqueueSnackbar(submitter.message, {
        variant: 'error',
  ...
  const updateFinalReport = async (file: any, reportId: string) => {
    ...
    const submitter = (await axiosFile.put(
      `${env.API_URL}/api/v1/final-reports/${reportId}`,
      body,
      {
        onUploadProgress: (axiosLoading: AxiosProgressEvent) => {
          setValueLoading(axiosLoading.progress);
